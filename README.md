...
0.3.0 在此前状态、上下文和历史分层设计上增加了 Observable Harness 与可信 Evaluator，但采用严格边界：
...
```text
cs-feat → cs-feat-design → design-review → impl → code-review → qa → accept
```
...
安装 `skills/` 到支持 Agent Skills 的宿主，然后在目标代码库执行：

```text
/cs init
```

也可以直接提交诉求；尚未初始化时 `/cs` 会同轮初始化并继续。
...
配置迁移会把旧 telemetry 设置存入 migration 备注，同时强制切换为 passive observation 与 manual evolution。旧 `.codestable/telemetry/runs/` 不会删除；可用迁移脚本 dry-run 后复制成新 observation：
